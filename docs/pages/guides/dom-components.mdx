...
You can use the `useDomImperativeHandle` hook inside a DOM component that to accept ref calls from the native side. The `useDomImperativeHandle` is similar to React's [`useImperativeHandle`](https://react.dev/reference/react/useImperativeHandle) hook, but it does not need a ref object to be passed to it.

```tsx App.tsx (native)
import { useRef } from 'react';
import { Button, View } from 'react-native';

import MyComponent, { type MyRef } from './my-componenet';
...
export default function MyComponent(props: React.RefAttributes<MyRef>) {
